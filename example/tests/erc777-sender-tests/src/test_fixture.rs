...
use casper_types::{account::AccountHash, bytesrepr::{Bytes, FromBytes, ToBytes}, runtime_args, AsymmetricType, CLTyped, ContractHash, Key, PublicKey, RuntimeArgs, U512, U256, HashAddr};

const ERC1820_CONTRACT_WASM: &str = "erc1820_registry.wasm";
const ERC777_CONTRACT_WASM: &str = "erc20_token.wasm";
const ERC777_SENDER_CONTRACT_WASM: &str = "erc777_sender.wasm";
const ERC1820_CONTRACT_NAME: &str = "erc1820_registry";
const ERC20_CONTRACT_NAME: &str = "erc20_token_contract";
const ERC777_SENDER_CONTRACT_NAME: &str = "erc777_sender_contract";
...
impl TestFixture {
    ...
    pub fn add_erc777_sender_context(&mut self) {
        println!("Install of erc777 sender");
        //----- ERC777 RECIPIENT
        let erc1820_contract_hash = self.contract_hash_erc1820();
        let erc777_contract_hash = self.contract_hash_erc20();
        let session_code = Code::from(ERC777_SENDER_CONTRACT_WASM);
        let session_args = runtime_args! {
            casper_erc777_sender::constants::ERC1820_CONTRACT_ARG_NAME => erc1820_contract_hash,
            casper_erc777_sender::constants::ERC777_CONTRACT_ARG_NAME => erc777_contract_hash
        };
    ...
    pub fn contract_hash_erc777_sender(&self) -> ContractHash {
        self.context
            .get_account(self.ali)
            .unwrap()
            .named_keys()
            .get(ERC777_SENDER_CONTRACT_NAME)
            .unwrap()
            .normalize()
            .into_hash()
            .unwrap()
    ...
    //------ ERC777 Sender Calls
    pub fn transfer_from_erc777_sender(
        &mut self,
        from: Key,
        to: Key,
        amount: U256,
        ...
        self.call(
            sender,
            self.contract_hash_erc777_sender().value(),
            casper_erc777_sender::constants::TRANSFER_ENTRY_POINT,
            runtime_args! {
                casper_erc777_sender::constants::FROM_RUNTIME_ARG_NAME => from,
                casper_erc777_sender::constants::TO_RUNTIME_ARG_NAME => to,
                casper_erc777_sender::constants::AMOUNT_RUNTIME_ARG_NAME => amount,
                casper_erc777_sender::constants::USER_DATA_RUNTIME_ARG_NAME => user_data,
                casper_erc777_sender::constants::OPERATOR_DATA_RUNTIME_ARG_NAME => operator_data
            },
        );
    }

    pub fn burn_from_erc777_sender(
        &mut self,
        account: Key,
        amount: U256,
        user_data: String,
        ...
        self.call(
            sender,
            self.contract_hash_erc777_sender().value(),
            casper_erc777_sender::constants::BURN_ENTRY_POINT,
            runtime_args! {
                casper_erc777_sender::constants::ACCOUNT_RUNTIME_ARG_NAME => account,
                casper_erc777_sender::constants::AMOUNT_RUNTIME_ARG_NAME => amount,
                casper_erc777_sender::constants::USER_DATA_RUNTIME_ARG_NAME => user_data,
                casper_erc777_sender::constants::OPERATOR_DATA_RUNTIME_ARG_NAME => operator_data
            },
